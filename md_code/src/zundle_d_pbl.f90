...
real :: s1,s2, bondlengthdifference, theangle
...
character(LEN=100) :: argv, in_file, out_file!='Zundel_000.dat'
    ...
    call system('mkdir -p output/dotp')
    out_file = 'output/dotp/'//'Zundel-'//trim(out_file)//'dat'
	...
	theangle = dot_product(averagevector1,averagevector2)/sqrt(dot_product(averagevector1,averagevector1) & 
	*dot_product(averagevector2,averagevector2))
	write(10,*)theangle,bondlengthdifference
